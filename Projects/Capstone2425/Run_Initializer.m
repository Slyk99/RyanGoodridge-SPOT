...
appHandle.AvailableDiagramsDropDown.Value = "Capstone_MPC_Final.slx";
...
% Edit BLACK LQR cost function
appHandle.BlackQXEditField.Value = 50;
appHandle.BlackQYEditField.Value = 50;
appHandle.BlackQThetaEditField.Value = 10;

appHandle.UpdateAuxParameters();
...
appHandle.SubPhase1EditField.Value = 2*baseRate;
appHandle.SubPhase2EditField.Value = 120*5;
...
appHandle.SubAppMassProperties.MassRedEditField.Value = 14.654;
appHandle.SubAppMassProperties.InertiaRedEditField.Value = 1.1*0.2378;
...
Q = diag([10, 10, 300, 300, 300, 500]);         % State error weighting matrix
R = diag([100, 100, 10]);                       % Control weighting matrix
...
eta = 0.2;                                      % Holding radius linear tolerance [m]
...
lidar_FOV = 40*d2r;                             % LiDAR Field-of-View [rad]
lidar_offset = [0.145, 0.087]';                 % LiDAR Offset (body-fixed) [m, m]
...
elseif test_case == 2
    u_0 = [0 0.14 0]';
    r_hold_0 = 1.05.*[1 1];
    x_RED_0 = [3.2 yLength-2.1 pi/2 0 0 0]';
    x_BLACK_0 = [1.6 yLength-2.1 3*pi/2 -0.0025 0.005 1*d2r]';
elseif test_case == 3
    u_0 = [0 -0.1 0]';
    r_hold_0 = 0.85.*[1 1];
    x_RED_0 = [3.2 2.1 pi/2 0 0 0]';
    x_BLACK_0 = [1.7 1.2 300*d2r 0 0 1.88*d2r]';
elseif test_case == 4
    u_0 = [-0.14 0 0]';
    r_hold_0 = 1.05.*[1 1];
...
theta_star_RED = rotateToFace(x_RED_0(3), x_BLACK_0(1:2)-x_RED_0(1:2) + rotz(x_RED_0(3))*lidar_offset) + docking_offset(3);
x_RED_0(3) = wrapAngle(theta_star_RED);
%theta_star_BLACK = rotateToFace(x_BLACK_0(3), x_BLACK_0(1:2)-x_RED_0(1:2)) + pi/2;
%x_BLACK_0(3) = wrapAngle(theta_star_BLACK);

%%

% Inital estimate/guess for UKF
x_est_0 = 1.1*x_BLACK_0;
x_est_0(3) = wrapAngle(x_est_0(3));
P_0 = diag(diag(x_est_0*x_est_0'));
