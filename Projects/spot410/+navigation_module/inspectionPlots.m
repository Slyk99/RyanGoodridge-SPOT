...
for i = 2:length(dataClass_rt.Time_s.Time)
    [~,~,~,debugOut] = SpotEstimator( dataClass_rt.SpotGnc_Phase.Data(i,:), ...
                                      ...
                                      dataClass_rt.SpotGnc_CtrlFwd.Data(i-1,:) + dataClass_rt.SpotGnc_CtrlKp.Data(i-1,:) + dataClass_rt.SpotGnc_CtrlKd.Data(i-1,:), ...
                                      paramEst, navOpts);
...
ProcRel.rangeLrf = dataClass_rt.SpotGnc_Proc.Data(:,SpotSensor.rLaser)/ 100;
...
% Extract 3σ bounds
offset = 9; % first 9 cols are states
n = 9;      % state dimension
